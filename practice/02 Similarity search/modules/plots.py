...
def plot_bestmatch_results(ts: np.ndarray, query: np.ndarray, bestmatch_results: dict) -> None:
    ...
    # Определяем длину запроса и временного ряда
    query_len = len(query)
    ts_len = len(ts)

    # Создаем фигуру с подграфиками
    fig = make_subplots(rows=1, cols=1, subplot_titles=("Best Match Results"), vertical_spacing=0.1)

    # Добавляем временной ряд
    fig.add_trace(go.Scatter(x=np.arange(ts_len), y=ts, mode='lines', name='Time Series',
                             line=dict(color='blue', width=2)))

    # Добавляем запрос
    fig.add_trace(go.Scatter(x=np.arange(query_len), y=query, mode='lines', name='Query',
                             line=dict(color='orange', width=2)))

    # Получаем индексы и расстояния из результата
    indices = bestmatch_results['indices']
    distances = bestmatch_results['distances']

    # Выделяем найденные совпадения
    for index in indices:
        # Добавляем подграфик для каждого найденного совпадения
        fig.add_trace(go.Scatter(
            x=np.arange(index, index + query_len), 
            y=ts[index:index + query_len], 
            mode='lines', 
            name=f'Match at {index}', 
            line=dict(color='red', width=2, dash='dash')
        ))

    # Настройки осей и заголовка
    fig.update_xaxes(showgrid=False, title='Time', title_font=dict(size=22, color='black'),
                     linecolor='#000', ticks='outside', tickfont=dict(size=18, color='black'),
                     linewidth=2, tickwidth=2)
    fig.update_yaxes(showgrid=False, title='Values', title_font=dict(size=22, color='black'),
                     linecolor='#000', ticks='outside', tickfont=dict(size=18, color='black'),
                     zeroline=False, linewidth=2, tickwidth=2)
